...
#SAVE AS SEPARATE PICTURES
for (var in unique(tot_frame$`Security Region`)) {
plots <- ggplot(subset(tot_frame, tot_frame$`Security Region` == var), aes(as.Date(Date), Value)) +
      geom_bar(stat = 'identity', color = "greenyellow") +
      labs(x = "Sample Date", y = "Reported number of cases per 100,000 inhabitants", title = paste0(var)) + scale_x_date(date_labels = "%B")
...
plot <- ggplot(tot_frame, aes(as.Date(Date), Value)) +
      geom_bar(stat = 'identity', color = "greenyellow") +
      labs(x = "Date", y = "Reported number of cases per 100,000 inhabitants") + facet_wrap(. ~ `Security Region`) + scale_x_date(date_labels = "%B") +
  theme_minimal()
