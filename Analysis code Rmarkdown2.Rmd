...
totalCalculator2 <- function(used_dates) {
  ...
  diff_frame <- as.data.frame(cbind(dates[-1], totals))
  #give the columns informative names
  colnames(diff_frame) <- c("Date", "Reported infections per 100.000")
  #and make the increase numeric
  diff_frame$`Reported infections per 100.000` <- as.numeric(diff_frame$`Reported infections per 100.000`)
  
   #create a plot of the increase per 100.000 inhabitants over time
   barplot(diff_frame$`Reported infections per 100.000`, 
...
#Step 3: the mean the level of virus particles found in the water per 100.000 cases
```{r}
...
rnaWeighting <- function(used_dates) {
  ...
  non_na_data2 <- non_na_data[!is.na(non_na_data$TP_security.region),]
  non_na_data2$TP_security.region <- as.double(non_na_data2$TP_security.region)
  non_na_data2$adjusted_RNA_flow <- (non_na_data2$TP_RNA.flow.per.100000 * non_na_data2$TP_security.region) #adjust rna flow by multiplying it with the proportion of water from that security region processed by that RWZI
...
```{r}
meanRNAcalculator <- function(used_dates, date) {
  dates <- unique(used_dates$newdate) #create a list of dates
  means <- numeric(length(dates))
  step1 <- filter(used_dates, newdate == date)
    
  step1$MN_name <- as.factor(step1$MN_name)
  mean_rna_flows <- tapply(step1$TP_RNA.flow.per.100000, list(Municaplities = step1$MN_name), sum, na.rm = TRUE)
...
#multiply by 1000 to get rid of the "." in the population variable
#dat <- dat %>% mutate(MN_pop = MN_pop*1000)
#i think this line in step 2 is double
