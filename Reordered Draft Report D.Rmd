...
Although patients do not always show symptoms of the virus, they do leave RNA particles of the virus in their feces as shown by for example @pan2020viral. The virus can sustain itself for a long period of time within the feces, in some cases even one or more months after the respective patient has tested negative for RNA particles in their feces [@vallejo2020highly]. Therefore, the amount of RNA particles could be an indicator of the true number of COVID-19 patients within a country or security region. 

Ever since the start of the pandemic in the Netherlands, the National Institute for Public Health and the Environment (RIVM) has been collecting samples from sewage treatment plants (STPs) and testing them for RNA presence. The research started small, with only 29 out of the 355 STPs in April 2020, but since the beginning of September of the same year, all STPs in the Netherlands are sampled once or multiple times a week. 
...
As RNA can be used as an indicator for estimating the true number of covid cases, this project attempts to assess whether RNA particles are a good predictor for the true number of covid cases. We work towards this goal through answering the following sub-questions:
\begin{APAenumerate}
  \setlength{\itemsep}{0pt}
  \setlength{\parskip}{0pt}
   \item How does the total number of reported infections in the Netherlands as a whole develop over time? 
   \item What does this trend look like displayed per 100,000 inhabitants of the Netherlands? 
   \item What is the mean level of RNA particles found in the water per 100,000 inhabitants? 
   \item How do RNA particles and the total number of infections relate to each other? 
\end{APAenumerate}

The remainder of this report is structured as follows. Prior research in the field of measuring RNA is discussed in Section \ref{sec:litrev}. Section \ref{sec:data} describes the data and the challenges they provided. The methodology used to answer the subquestions, as well as the results are discussed in Section \ref{sec:metresults}. Section \ref{sec:conclusion} interprets and aggregates the results and subsequently provides a conclusion. Finally, Section \ref{sec:discussion} discusses certain things that could have gone better in this research.  
...
We are working with two datasets, both collected by the Dutch National Institute for Public Health and the Environment (RIVM). The first dataset contains the total number of positive tests reported per day per municipality. It also contains information on key characteristics of the municipality, such as population density and which security region it is part of. The second dataset contains data recorded on the level of STPs. The key variable here is the average concentration of SARS-CoV-2 RNA measured in the daily amount of sewage water per 100,000 inhabitants. This dataset also contains crucial metadata of the STPs, such as in which security region the area of responsibility of this STP falls. The two datasets were matched to each other by the variable in which security region a municipality and a treatment plant’s area fall respectively.
...
However, some STPs also treat water from outside their primary security region, creating double entries in the dataset and making simple matching impossible. Luckily, the dataset also provides information on what percentage of the water a STP processes comes from which security region, so we can weight the RNA flow by this variable. See SR2 of Figure \ref{fig:diagram}.
...
We take the above relationships into account in our analyses. Furthermore, we conclude that we have to aggregate our data to the Security Region level when establishing a relationship between RNA flow and number of positive cases, to get an accurate indicator. Moreover, the data we use in our analyses are a subset of the original dataset, because the dataset only includes all STPs from the 7th of September onwards. 

Another problem with a longitudinal study such as this one is that missing values can occur at any given point during the study. This could subsequently affect the final results in a negative way. Because of the missing data, not every security region has data on RNA flow for one day and data on reported cases seven days later. As this would be the ideal situation, the missing RNA flow data will be imputed. More specifically, Last Observation Carried Forward (LOCF) imputes the missing data due to the scope of the project. LOCF is a simple imputation method that imputes missing data for an individual by assigning the last observed value for that individual to each missing value [@cook2004marginal]. Although this does solve the problem of missing data, it is not the best imputation method and it does have some limitations.

First of all, it is a conservative imputation method according to @streiner2008missing. This means that it underestimates what the missing RNA flow values would have been as this technique assumes that an individual does not change at the times of missing values. Therefore, LOCF introduces some bias in the estimates due to this underestimation. Nevertheless, it would still provide better estimates than using only the observed cases for the analyses. Second, LOCF has an ad hoc nature which means that it has no measure of uncertainty concerning the imputed values [@kunzmann2020imputation]. This implies that no additional covariates can be included to reduce bias introduced by this imputation method. 

\section{Methodology and Results}
\label{sec:metresults}

In this section, the different subquestions are dealt with consecutively. First, the methodology that was used to answer the specfic question is described. Second, the results are displayed and discussed immediately afterwards. 

\subsection{How does the total number of reported infections in the Netherlands as a whole develop over time?}
\label{sec:sub1}

In order to answer the first question, we needed a function that calculated the number of total reported infections per day. A function was written that aggregated the data by the number of reported infections in a municipality on a given day to the total number of reported infections on that day in the Netherlands as a whole, discarding double entries per municipality. The resulting output is visualized in Figure \ref{fig:2a}, where you can see the total number of reported infections we have had over time in the Netherlands. The second wave is also clearly visible, where it can be seen that the total number of reported infections increasing more rapidly in October compared to September. 

We were also interested in the daily fluctuations of the number of reported infections in the Netherlands. To show this, we altered the function such that it would calculate the difference in total reported cases between each day and the day before. The results are visualized in Figure \ref{fig:2b}. We see that the peak of new infections in the second wave is somewhere at the end of October, as we would expect from the previous figure.  
...
Next, we examined if this trend differs between the various security regions in the Netherlands. We calculated both the cumulative number of reported cases and the difference in number of reported cases per day for each of the 25 security regions. To be able to make a meaningful comparison between the different regions, we calculate these indicator per 100,000 inhabitants, as the population density differs between security regions. Below we show a selection of the resulting graphs. 
...
plot <- ggplot(tot_frame, aes(as.Date(Date), Value)) +
      ...
      labs(x = "Date", y = "Reported number of cases per 100,000 inhabitants per security region") + facet_wrap(. ~ `Security Region`) + scale_x_date(date_labels = "%b") +
  theme_minimal()
...
\centering
    ...
    \caption{Number of cases per 100,000 inhabitants per security region}
    \label{fig:nice}
\end{figure}

As can be seen in \ref{fig:nice}, the number of cases per 100,000 inhabitants varies per security region. For example, the number of cases in Amsterdam-Amstelland is much higher than in Zeeland. Moreover, the increase in cases had a different trend in Rotterdam-Rijnmond compared to Amsterdam-Amstelland. 
...
diffperregion <- function(tot_frame) {
  ...
  #WRAP IN ONE FRAME
plot <- ggplot(df, aes(as.Date(Date), Value)) +
      geom_bar(stat = 'identity', color = "greenyellow") +
      labs(x = "Date", y = "New number of reported cases per 100,000 inhabitants per security region") + facet_wrap(. ~ `Security.Region`) + scale_x_date(date_labels = "%b") +
  theme_minimal()
...
We saw that the number of infections had been increasing since the beginning of September and peaked in October. As one only requests a test after experiencing symptoms, one would expect the amount of RNA particles in the sewage water to precede the increase in the number of infections. Therefore, we visualized the distribution of the RNA particles in the Netherlands over time. This figure can be seen in the appendix. The shape of the plot loosely follows the same trend as the plot of the reported infections per day, but its shape is much less smooth. The numbers of the amount of RNA particles in the sewage per day fluctuate more than the number of new infections. 

The aforementioned plot shows the total number of RNA particles per day. The number of installations included differs each day, which can be seen in the appendix. This may be an explanation for the fluctuation in the amount of RNA particles measured each day. To correct for this we also plotted the mean level of RNA particles in the sewage per day. The plot, which can be seen in the appendix, is smoother than the plot that shows the totals. However, there is still some fluctuation. If in all installations the same amount of RNA particles were found, you would not expect fluctuations. In that case the mean level is not affected by a missing installation. This suggests that in different installations a different amount of RNA particles can be found. Since different installations care for different regions, different regions may have different trends. In order to explore this we plotted the total amount of RNA particles found in the sewage for per day for each region. 
As can be seen in the figure, the trend of the amount of RNA particles found differs per security region. For example, the amount of RNA particles found in Friesland is lower than in Midden- en West-Brabant. Also, in Friesland there is not really a peak, whereas there is clear peak in Midden- en West-Brabant. The trends per region look similar to the overall trend. They loosely follow the trend of infections, but there are still fluctuations. 
...
\subsection{How do RNA particles and the total number of infections relate to each other?}
\label{sec:sub4}
...
```{r 9, message=FALSE, warning=FALSE, results = 'hide', fig.align = "center", fig.cap = x11, fig.height = 10, fig.width = 10, fig.pos = "H"}
x11 <- paste(
...
```{r 10, results = 'hide', fig.align = "center", fig.cap = x12, fig.height = 10, fig.width = 10, fig.pos = "H"}
x12 <- paste(
...
When we started our quest, our aim was to build a model around RNA particles in the sewage in order to predict COVID-19 infections. We soon realized that this might be unrealistic. In order to find out how the amount of RNA particles in the sewage and COVID-19 infections are related we had to conduct a whole research on its own. We therefore shifted our focus to understanding this relationship. We regard this report as a stepping stone to a model that can predict the number of COVID-19 cases.

As any research, our research has limitations. Our original dataset contained many missing values. We dealed with this by applying LOCF. This method has several drawbacks. It is a conservative method according to @streiner2008missing. More specifically this means that the estimates of the amount of RNA particles are an underestimation. A second problem wth the method is that it does not provide a measure of uncertainty concerning the imputed values [@kunzmann2020imputation]. So we do not know how wrong the estimate is. 

A second limitation of our study is that we discarded several outliers. In our analysis of the relationship between COVID-19 infections and RNA particles in the sewage we stumbled upon extreme values. These values had such an impact on the correlation for the particular region that we decided to discard them. We could not find out what these outliers were caused by. So it is possible that we threw away relevant information. 

Third, we are still unable to explain why the trend of RNA particles in the sewage per region fluctuates.Something else we could not explain are the different correlations between RNA particles in the sewage and COVID-19 infections on regional level. The fluctuations and the the different correlations may be caused by the same thing that is manifest on the regional level. Future research could focus on this.
...
```{r 4a, results = 'hide', fig.align = "center", fig.cap = x5, fig.height = 4, fig.width = 5.5, fig.pos = "H"}
x5 <- paste(
...
```{r 4, results = 'hide', fig.align = "center", fig.cap = x7, fig.height = 4, fig.width = 5.5, fig.pos = "H"}
x7 <- paste(
...
```{r 7, results = 'hide', fig.align = "center", fig.cap = x9, fig.height = 4, fig.width = 5.5, fig.pos = "H"}
x9 <- paste(
    "Relation between RNA flow and increase in infections for one timestamp")
plot(day_frame$RNA.flow, day_frame$Reported, col = "greenyellow", pch = 19, xlab = "RNA flow", ylab = "Increase in number of infections")
...
```{r 8, results = 'hide', fig.align = "center", fig.cap = x10, fig.height = 4, fig.width = 5.5, fig.pos = "H"}
x10 <- paste(
...
#Plot with the 3 outliers we removed coloured red
```{r}
ggplot(better_frame, aes(x = RNA.flow, y = Increase)) +
  geom_point(stat = 'identity', color = "greenyellow") +
  geom_point(data = outlier_frame, stat = 'identity', color = "red") +
  labs(title = "Correlation between RNA flow and increase in nr of cases per region", subtitle = "Three outliers we removed colored red", x = "RNA flow", y = "Increase") + 
