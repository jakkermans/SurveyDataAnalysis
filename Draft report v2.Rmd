...
```{r}
setwd("D:/Master Utrecht/Survey Data Analysis/SurveyDataAnalysis-main/")
...
After the samples are taken, they are transported at a controlled temperature to the RIVM, where they are analysed by researchers for RNA particles. RNA is isolated and Polymerase Chain Reaction (PCR) is performed on the samples to determine the amount of RNA particles present in the wastewater (RIVM, 2020).
...
Another problem with the current data on RNA in sewage water, is that it is unknown how much RNA particles need to be shed, for them to be detected during testing (CDC, 2020). This complicates prediction, as some RNA may go undetected, resulting in an underestimate of the total number of infections in the Netherlands as a whole (or a single region, for that matter). 
...
totalCalculator <- function(used_dates) {
  ...
  plot <- barplot(totals_frame$`Total number of reported infections`, 
                  col  = "greenyellow",
                  main = "Figure 2a: Total number of reported infections per day in the Netherlands over time", 
                  xlab = "Date", 
...
differenceCalculator <- function(used_dates) {
  ...
  plot <- barplot(diff_frame$`Difference in reported infections`, 
                  col = "greenyellow",
                  main = "Figure 2b: Number of new reported infections per day in the Netherlands over time", 
                  xlab = "Date", 
...
totalCalculator2 <- function(used_dates) {
  ...
  barplot(tot_frame$`Reported infections per 100.000`, 
          col = "greenyellow",
          main = "Figure 3a: Total number of reported infections per day per 100,000 inhabitants in the Netherlands over time", 
          xlab = "Date", 
...
differenceCalculator2 <- function(used_dates) {
  ...
  barplot(diff_frame$`Increase in reported infections per 100.000`, 
          col = "greenyellow",
          main = "Figure 3b: Number of new reported infections per day per 100,000 inhabitants in the Netherlands over time", 
          xlab = "Date", 
...
virusCalculator_new <- function(used_dates) {
  ...
  #create a barplot 
  barplot(means_frame$`Mean RNA flow per 100000`, col='greenyellow', main = 'Figure 4: Mean RNA flow per 100,000 inhabitants in the Netherlands per day', 
...
rnaWeighting <- function(used_dates) {
  ...
  non_na_data2$adjusted_RNA_flow <- (non_na_data2$TP_RNA.flow.per.100000.locf *    non_na_data2$TP_percentage.in.SR.locf) #adjust rna flow by multiplying it with the proportion of water from that security region processed by that RWZI
...
plot(day_frame$RNA.flow, day_frame$Reported, col = "red", pch = 19, xlab = "RNA flow", ylab = "Increase in number of infections", main = "Figure 5: Relation between RNA flow and increase in infections for one timestamp"); abline(lm(Reported ~ RNA.flow, data = day_frame), lwd = 2)
...
### Step 5: Correlations over the period 8th of September till 10th of november
```{r fig.width = 10,echo = FALSE}
...
plot(x = corr_frame$Date, y = corr_frame$Correlation, col = "red", pch = 19, xlab = "Date", ylab = "Correlation", main = "Figure 6: Correlations between RNA flow and increase in infections over time"); abline(lm(Correlation ~ Date, data = corr_frame), lwd = 2)

corr_frame2 <- corr_frame[!abs(corr_frame$Correlation) > 1.5 * IQR(corr_frame$Correlation),]
plot(x = corr_frame2$Date, y = corr_frame2$Correlation, col = "red", pch = 19, xlab = "Date", ylab = "Correlation", main = "Figure 6: Correlations between RNA flow and increase in infections over time"); abline(lm(Correlation ~ Date, data = corr_frame2), lwd = 2)

mean(corr_frame2$Correlation, na.rm = TRUE)
boxplot(corr_frame$Correlation)
```
...
```{r waste bin, fig.width = 10, fig.height=10, echo = FALSE}
#CODE TO PLOT A MAP WITH SECURITY REGIONS
...
mun_dat3 <-subset(mun_dat2, mun_dat2$municipality %in% used_dates$MN_name)
...
dat2 <- used_dates %>% select(gemeente = `MN_name`, Regio = `SR_name`)

names_and_numbers <- data_frame(id=rownames(NLD@data),
                                gemeente=NLD@data$NAME_2) %>% 
  left_join(dat2, by = "gemeente")
...
ggplot(final_map)+
  ...
  geom_point(data = mun_dat3, aes(x = as.numeric(longitude), y = as.numeric(latitude), colour = SR_name))
```
